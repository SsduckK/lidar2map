...
class Optimizer_lr(object):
    ...
    def get_lr(self):
        if self.it <= self.warmup_steps:
            ...
            factor = (1-(self.it-self.warmup_steps)/(self.max_iter-self.warmup_steps))**self.power
            lr = (self.lr0-self.lrend) * factor + self.lrend
        return lr
...
class Optimizer_pre(object):
    def __init__(self,
        ...
        wd_params, nowd_params, lr_mul_wd_params, lr_mul_nowd_params = model.get_params_pre()
        param_list = [
                {'params': wd_params},
                {'params': nowd_params, 'weight_decay': 0},
                {'params': lr_mul_wd_params, 'lr_mul': True},
