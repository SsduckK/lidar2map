...
class OdomCorrector(Node):
    def __init__(self):
        ...
        transformed_msg = message_filters.Subscriber(self, TransformStamped, "/base_footprint")
        odom_msg = message_filters.Subscriber(self, Odometry, "/odom")
        self.base_odom_publsiher = self.create_publisher(Odometry, "/new_odom", 1)
        self.prv_pose = [0, 0, 0, 0]
        self.outlier_cnt = 0
        self.frame_cnt = 0
        filtered_msg = message_filters.ApproximateTimeSynchronizer([transformed_msg, odom_msg], 1, 0.2, allow_headerless=True)
        filtered_msg.registerCallback(self.callback)

    def callback(self, tf, odom):
        ...
        tf_matrix, odom_matrix = np.identity(4), np.identity(4)
        ...
        new_matrix = tf_matrix@odom_matrix
        self.pub_new_odom(time, new_matrix)

    def tf_callback(self, tf):
        transform = tf.transforms
        tf = transform[0]
        self.transform_pub.publish(tf)

    def pub_new_odom(self, time, matrix):
        rotation = matrix[:3, :3]
    ...
    def draw_point(self, matrix):
        ...
        if self.frame_cnt % 20 == 0:
            print("saved")
            plt.savefig("/home/ri/colcon_ws/src/lidar2map/data/new_odom_base_2.png")
